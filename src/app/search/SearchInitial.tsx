...
export function SearchBox({handleSearch}: {handleSearch: (ref: RefObject<HTMLTextAreaElement>) => Promise<void>}) {
    ...
    return (
        ...
        className="bg-slate-50 border-4 border-teal-200  focus:border-teal-400 focus:outline-teal-400 resize-none pl-6 pr-6 pt-3 pb-3 ml-4 mr-4 h-full w-full rounded-full text-lg font-mono"
...
export function SearchInitial({handleSearch, getRecs}: {handleSearch: (ref: RefObject<HTMLTextAreaElement>) => Promise<void>, getRecs: () => Promise<void>}) {
    ...
    return (
        <div className="flex flex-col justify-evenly items-center w-full max-h-screen bg-white">
            <div className="min-h-[80px] h-[20%] aspect-square mt-20 mb-20">
            ...
            <div className="h-[20%] flex justify-center items-center min-w-[400px] w-[50%] text-black">
                <SearchBox handleSearch={handleSearch}/>
            </div>
            {state.recs?
            <div className="flex flex-wrap justify-center py-4 bg-teal-50 rounded-lg border-2 border-teal-400 h-full w-1/2 min-w-[400px] mt-20 mb-20 font-mono overflow-y-scroll no-scrollbar text-black">
                {state.recs.map((rec: any, i: number) => (
                    <div key={i} className="flex flex-col bg-slate-100 border-2 border-slate-300 my-2 mx-2 p-4 rounded-lg w-[45%]">
                        <a href={rec.link} target="_blank" rel="noopener noreferrer" className="underline bg-teal-100 bg-opacity-0 hover:bg-opacity-100">
                            {rec.title}
                        </a>
                        <div className="grow">
                            {rec.snippet}
            ...
            :
            <div className="flex flex-wrap h-full w-1/2 min-w-[400px] mt-20 mb-20"></div>
            }
        </div>
    );
}